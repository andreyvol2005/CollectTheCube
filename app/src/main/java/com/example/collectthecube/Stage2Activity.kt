...
class Stage2Activity : AppCompatActivity() {
    ...
    private fun loadContentFromFirebase() {
        ContentLoader.loadStageContent("stage2",
    ...
    private fun createImageGridItem(item: Map<String, Any>): LinearLayout {
        return LinearLayout(this).apply {
            ...
            // Image
            addView(ImageView(this@Stage2Activity).apply {
                layoutParams = LinearLayout.LayoutParams(
                    (item["image_size"] as? Int ?: 100).dpToPx(),
                    (item["image_size"] as? Int ?: 100).dpToPx()
                )
            ...
            if (title.isNotEmpty()) {
                addView(TextView(this@Stage2Activity).apply {
                    text = title
                    textSize = 14f
                    setTypeface(typeface, Typeface.BOLD)
    ...
    private fun updateButtonState(isCompleted: Boolean) {
        markCompletedButton.apply {
            ...
            setBackgroundColor(ContextCompat.getColor(this@Stage2Activity,
