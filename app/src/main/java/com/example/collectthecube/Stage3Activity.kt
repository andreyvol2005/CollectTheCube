...
class Stage3Activity : AppCompatActivity() {
    ...
    private fun loadContentFromFirebase() {
        ContentLoader.loadStageContent("stage3",
    ...
    private fun createImageGridItem(item: Map<String, Any>): LinearLayout {
        return LinearLayout(this).apply {
            ...
            // Image
            addView(ImageView(this@Stage3Activity).apply {
                layoutParams = LinearLayout.LayoutParams(
            ...
            // Title (если не пустой)
            val title = item["title"] as? String ?: ""
            if (title.isNotEmpty()) {
                addView(TextView(this@Stage3Activity).apply {
                    text = title
                    textSize = 14f
                    setTypeface(typeface, Typeface.BOLD)
    ...
    private fun updateButtonState(isCompleted: Boolean) {
        markCompletedButton.apply {
            ...
            setBackgroundColor(ContextCompat.getColor(this@Stage3Activity,
